...
class ManejoXML {
	 ...
	 */
	@Keyword
	def devuelveVariableporTag(TestObject request, String nombreVariable) {
		if (request instanceof RequestObject) {
			...
			def parsedXML = new XmlSlurper().parseText(responseText)
			def status = parsedXML.'**'.findAll { it.name() == nombreVariable}
			def varporTag= status.toString().replace("[", "").replace("]", "")
			return varporTag
		} else {
	 ...
	 @Keyword
	 def devuelveVariableporTag(TestObject request, String nombreVariable, String nombreVariable2) {
	 if (request instanceof RequestObject) {
	 RequestObject requestObject = (RequestObject) request
	 ResponseObject response = WSBuiltInKeywords.sendRequest(requestObject)
	 def responseText= response.getResponseText()
	...
	@Keyword
	def devuelveVariableporJson(TestObject request, String nombreVariable) {
		if (request instanceof RequestObject) {
			RequestObject requestObject = (RequestObject) request
			ResponseObject response = WSBuiltInKeywords.sendRequest(requestObject)
			def responsePreParsed2 = response.getResponseText()
			def manejojson= new JsonSlurper().parseText(responsePreParsed2)
			return manejojson[nombreVariable]
		} else {
			KeywordUtil.markFailed(request.getObjectId() + " is not a RequestObject")
		}
	}
